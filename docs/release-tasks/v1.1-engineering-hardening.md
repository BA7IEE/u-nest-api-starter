...
> **🗄 归档说明 — 不再作为当前执行指令**
>
> - **原路径**:仓库根 `TASKS.md`(v0.1.7 起归档至 `docs/release-tasks/v1.1-engineering-hardening.md`)
> - **历史定位**:本文是 V1.1 工程加固阶段的任务清单,9 个任务全部 ✅ 已完成(对应 release `v0.1.1`),内容作为历史快照保留
> - **当前模板状态**:见 [`../../README.md`](../../README.md) 顶部 `Template baseline` / `template-freeze` 声明
> - **当前长期规则**:见 [`../../ARCHITECTURE.md`](../../ARCHITECTURE.md) / [`../../CLAUDE.md`](../../CLAUDE.md) / [`../../AGENTS.md`](../../AGENTS.md) / [`../development.md`](../development.md) / [`../testing.md`](../testing.md) / [`../deployment.md`](../deployment.md) / [`../security.md`](../security.md)
> - **代码注释中的旧引用**:`src/**` / `test/**` / `.github/workflows/ci.yml` 中仍有形如 `// V1.1 TASKS.md 15.x` 的注释,作为代码上下文标签保留;按本文件阅读即可对应到原任务编号
> - **历史文本中保留的旧数字**(如"v1 已有 137 用例"):反映当时事实,不代表当前测试数量;**禁止以此为不变式假设**

---

## 原始正文

> 本文件是 V1.1 工程加固的**唯一执行清单**(历史定位)。规范定义见 `ARCHITECTURE.md` §11、`AGENTS.md` §17、`CLAUDE.md` §17。
