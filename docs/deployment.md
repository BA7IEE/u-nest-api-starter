...
V1.1 在 v1 业务接口与数据模型不变的前提下补齐基础工程能力。规范定义见 [`ARCHITECTURE.md`](../ARCHITECTURE.md) §11、[`AGENTS.md`](../AGENTS.md) §17、[`CLAUDE.md`](../CLAUDE.md) §17;历史任务清单(已收尾)见 [`release-tasks/v1.1-engineering-hardening.md`](./release-tasks/v1.1-engineering-hardening.md)。
...
`<version>` 占位符表示派生项目的 release 版本号(模板自身当前为 `0.1.7`,见 [`../package.json`](../package.json) `#version`)。派生项目按自己的 release tag 替换。

```bash
docker build -t u-nest-api-starter:<version> .
```
