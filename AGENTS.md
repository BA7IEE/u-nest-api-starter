# AGENTS.md — 通用 AI Agent 项目协作铁律(v1 + v2.0 baseline)

> 给所有 **AI 编码助手 / Agent**(Cursor、GitHub Copilot Chat、Continue、Cline、其他 CLI / IDE Agent)看的项目工作铁律,从 `ARCHITECTURE.md` §7 与附录抽取。
> 修改本仓库代码前必须读完:`ARCHITECTURE.md`(v1 蓝图 + v2.0 baseline)+ `AGENTS.md` + [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md);如使用 Claude Code,还必须读 `CLAUDE.md`。
> 本文与 `CLAUDE.md` 内容**保持同步、并存**,差异仅在入口表述。如本文与 `ARCHITECTURE.md` 表述冲突,**以 `ARCHITECTURE.md` 为准**;v1 / v2.0 表述冲突时,**以 ADR-001 与 ARCHITECTURE.md §12 为准**。
...
> - **三档红线分级**:永久红线([`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §A,扩展至 29 条)/ v2.0 baseline 能力(本文 §18 与 `ARCHITECTURE.md` §12)/ 派生项目 ADR 解锁能力([`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §B)
...
## 18. 通用 AI Agent v2.0 baseline 执行约束(RBAC + Auth)

> 本节是通用 AI Agent 在 v2.0 baseline 阶段的特定执行约束,**优先级高于 §17**(§17 是 V1.1 历史阶段约束,已收尾)。
> 修改 v2.0 相关代码前,**必须**:
...
每个 v2.0 任务完成后,通用 AI Agent **必须**逐项验证再报告完成:
...
v2.0 baseline 完成后,**不自动触发** §18.3 中的任何 B 类解锁。任何"RBAC 接进来了顺手加 CASL""refresh 接进来了顺手加 access 黑名单"的延伸,都必须重新评估业务诉求并经用户明确确认,通用 AI Agent **不自行判定**。
