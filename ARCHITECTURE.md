# AI 友好的 TypeScript API 底座 — v1 蓝图(+ v2.0 baseline 升级公告)
...
>
> 本文档主体仍是 v1 蓝图。**v2.0 baseline 已把 RBAC(User → Role → Permission)+ refresh token + logout 升级为模板必备能力**,不再是禁止项。
>
> - 设计决策与影响范围:[`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md)
> - 红线分级:**永久红线** / **v2.0 baseline 能力** / **派生项目 ADR 解锁能力** 三档,见 [`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md)
> - 本文档 §1 末条 / §4 不做表 / §5 数据模型 / §6 接口清单 / §7.11 角色层级 / §8 环境变量 / §9 升级路径 / §12 v2.0 baseline 升级 各章节均已加入 v2.0 边注;主体内容保留供 v1.x 维护分支参考
> - **Phase 0(当前阶段)只改文档**,不动 `src` / `prisma/schema.prisma` / `package.json`;不创建 migration;Phase 1 才落地代码与 schema
> - v1 历史限制(如"不预先做 RBAC""不预先做刷新 token")**不再继续约束 v2.0**
...
12. [**v2.0 baseline 升级(RBAC + Auth)**](#12-v20-baseline-升级rbac--auth)
...
- **不预先做多租户**:多租户对底座侵入大、与业务强耦合(数据隔离规则、跨租户访问权限、组织树深度差异极大),先观察具体业务再加(见 §9 升级路径)。**RBAC 与刷新 token 在 v1 历史阶段曾被列入本条**,**v2.0 baseline 已把它们升级为底座必备能力**,不再适用本条限制(见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md))。
...
> **v2.0 baseline 升级提示**:本表是 v1 阶段的"不做"清单。下列项目在 v2.0 baseline 已升级为模板必备能力,不再适用"不做":
>
> - **RBAC**(`User → Role → Permission` 四表)→ v2.0 baseline,见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md) §4
> - **刷新 token + logout + rotation** → v2.0 baseline,见 ADR-001 §4.7
>
> 其余项目(本人改密码 / 多租户 / 文件上传 Provider / LLM 等)在 v2.0 仍维持"不做",派生项目按 [`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §B 走 ADR 解锁。

| 砍掉的功能 | 什么时候再加 |
|---|---|
| RBAC(permission 表 / 按钮级权限 / casl) | **v2.0 baseline 已升级**(不引入 CASL,自写 `Set.has() + *`;见 ADR-001 §4)。三层 Role enum 在 v1 历史阶段是"不做"清单的解释,v2.0 已被 `User → Role → Permission` 取代 |
| 文件上传具体实现(本地/OSS/R2) | 第一个产品真要传文件时(B-3) |
| Redis / 队列 / 定时任务 | 真有异步任务、限流、缓存需求时(B-10) |
| 注册接口 | 几乎不会加(都是管理员创建账号) |
| 刷新 token | **v2.0 baseline 已升级**(短 TTL access + 长 TTL refresh + rotation + logout,见 ADR-001 §4.7)。v1 阶段"真有无感续期诉求"的判定不再适用 v2.0 |
| **本人改密码接口**(`PUT /api/users/me/password`) | 真出现"普通用户用账密登录、需要自助改密码"产品时(B-8,**v2.0 baseline 仍不做**,配套决策多见 ADR-001 §7.3) |
| 微信小程序登录 | 第一个小程序产品要接时(B-9) |
| 多租户、组织树 | 救援队系统启动时(B-2) |
| LLM/向量检索 | 第一个用到 AI 的产品启动时(B-11) |
| 操作日志 / 登录日志 | 真有审计需求时(B-6) |
| 字典管理 | 真有"前端枚举要后台可配"诉求时(B-5) |
| 部分通配权限(`user:*` / `*:read`)、`@AnyPermission` OR、数据级 / 行级权限、角色继承 / 部门范围 | **v2.0 baseline 仍不做**,派生项目按 B-12 / B-13 / B-14 / B-15 走 ADR 解锁 |

---

## 5. 数据模型

> **v2.0 baseline 升级提示**:本节描述 v1 数据模型(仅 `User` 一张表 + `Role` / `UserStatus` 两个 enum)。v2.0 baseline 已扩展为:
> - 删除 `User.role` 字段,删除 `enum Role`
> - 新增 RBAC 四表:`Role` / `Permission` / `UserRole` / `RolePermission`
> - 新增 Auth 一表:`RefreshToken`(`tokenHash` sha256 存储)
> - 保留 `User` 其余字段 + `enum UserStatus` + `@@index([deletedAt])` / `@@index([status])`
>
> 完整 schema 草案见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md) §4.1。本章节保留 v1 模型描述供 v1.x 维护分支参考;Phase 1 落地后会同步更新本章节正文。
...
> **v2.0 baseline 升级提示**:本节描述 v1 阶段的 13 条接口。v2.0 baseline 已变更如下(完整契约见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md) §4.8):
>
> - `PATCH /api/users/:id/role` **删除**(单值角色废弃)
> - 新增 `PUT /api/users/:id/roles`(覆盖式分配,入参 `{ roleCodes: string[] }`,需 `user_role:assign` 权限)
> - 新增 `/api/roles*` 路由族(CRUD + `PUT /api/roles/:id/permissions`)
> - 新增 `GET /api/permissions`(只读)
> - `POST /api/auth/login` 出参从 `{ accessToken }` 改 `{ accessToken, refreshToken, accessExpiresIn, refreshExpiresIn }`
> - 新增 `POST /api/auth/refresh`(rotation 强制)与 `POST /api/auth/logout`(吊销当前 refresh)
> - `GET /api/users` / `:id` 出参 `role` 字段改 `roles: string[]`,且**不再按角色过滤可见范围**(`admin` 能看到 `super_admin` 行,语义变化)
> - `POST /api/users` 入参 `role?` 改 `roleCodes?: string[]`,缺省 `['user']`;显式非缺省值需调用者拥有 `user_role:assign`(防权限提升,见 ADR-001 §9.4)
> - `/api/users/me` / `GET /api/health*` 行为不变
>
> 本章节表格保留 v1 契约供 v1.x 维护分支参考。
...
> **🚨 v2.0 baseline 升级 — 本节已被 ADR-001 取代**(2026-05-18)
>
> 本节原文是 **v1 历史模型**(三层 `Role` enum + `@Roles` + `RolesGuard`)的权威描述,**v2.0 baseline 已升级为标准 RBAC**(`User → Role → Permission` 四表 + `@Permissions` + `PermissionsGuard`),见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md) §4。
>
> v2.0 关键变更:
...
> - `RolesGuard` → `PermissionsGuard`,全局注册顺序不变(`JwtAuthGuard → PermissionsGuard`)
> - 三个 baseline 角色 `super_admin` / `admin` / `user`(seed 注入,`isSystem=true`)
> - 七条 baseline 权限:`*` / `user:read` / `user:write` / `user_role:assign` / `role:read` / `role:write` / `permission:read`
> - **五条 super_admin 铁律**(ADR-001 §4.5)展开自本节"最后一个 SUPER_ADMIN 保护"
> - **不引入 CASL**,不做"按钮级 / 字段级 / 资源级 / 行级权限",权限码命名严格 `<resource>:<action>` 两段式
>
> **派生项目 v1.x 仍可参考本节作为历史模型**;v2.0 之后的实施以 ADR-001 为准。下方原文保留,**不再作为 v2.0 baseline 的权威依据**。
>
> ---
>
> **历史声明(v1 阶段):本项目的三层角色是"粗粒度角色等级",不是 RBAC(基于角色的访问控制)。**
>
> v1 阶段反复强调:AI 或新人看到 `Role` 枚举不要把它当作"已经在做 RBAC 的起点"逐步扩展(加 permission 表、加 user_roles 多对多、加 casl/ability 库),也不要追加按钮级、字段级、资源级权限点。这是一次架构升级,不是渐进改造。
>
> **这次架构升级在 v2.0 baseline 中已经发生**(ADR-001),v1 历史的"不渐进改造"原则在升级到 v2.0 时已通过一次性 baseline 重置完成,**不是被违反**。
>
> | 维度 | v1 历史模型(本节描述) | v2.0 baseline(ADR-001) |
> |---|---|---|
> | 角色定义 | Prisma `enum Role` 三层固定 | `Role` 表 + `isSystem` 三个 baseline 角色 + 派生可加 |
> | 用户-角色关系 | `User.role` 单值字段 | `UserRole` 多对多表,可多角色 |
> | 权限粒度 | 接口级(`@Roles(...)`)+ service 层目标用户校验 | 接口级(`@Permissions(...)`)+ service 层业务铁律(五条 super_admin 铁律 + 自我保护 + 防权限提升) |
> | 权限存储 | 硬编码在 `users.service.ts` / `users.policy.ts` | `Permission` 表 + `RolePermission` 映射(seed 注入,代码契约) |
> | 适用场景 | 内管系统"管理员管员工"简单层级 | 内管系统多角色、客户项目可后台分配角色、小程序后台 |
>
> v1 阶段的判断"99% 的内部管理系统、小程序后台、客户项目二开都不需要 RBAC"经实际派生项目验证**部分修正**:RBAC 本身(多角色 + 后台可配)是大多数派生项目都需要的能力,**不**需要的是更复杂的"按钮级 / 资源级 / 行级权限",这些 v2.0 仍然不做,走 ADR B-12 / B-14 / B-15 解锁。
...
# v1 历史变量(已弃用,v2.0 baseline 拆分为 access + refresh,见 ADR-001 §4.7)
# JWT_EXPIRES_IN=7d
# v2.0 baseline:
...
> **v2.0 baseline 升级提示**:本节描述 v1 阶段的"升级触发条件 → 该加什么"映射。**v2.0 baseline 已把 RBAC 与 refresh token 升级为底座必备能力**,故下表中"真要做权限点到按钮级"与"真有无感续期诉求"两行**已删除**(从 v2.0 起不再适用);其余条目继续生效,派生项目按 [`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §B 走 ADR 解锁。
...
| 出现"普通用户自助改密码"产品 | 加 `PUT /api/users/me/password` + `ChangeMyPasswordDto` + 防爆破;是否吊销其他设备 token 由该产品安全策略决定(B-8 保留) | `src/modules/auth/` + `src/modules/users/` |
...
| 需要部分通配权限(`user:*` / `*:read`) | 扩展 `PermissionsGuard` 通配匹配规则 | B-12(新),走 ADR |
| 需要 `@AnyPermission(...)` OR 语义 | 新增装饰器 + Guard 分支 | B-13(新),走 ADR |
| 需要数据级 / 行级权限(如"ADMIN 仅看自己创建的用户") | 各 service 按 actor 过滤 | B-14(新),走 ADR |
| 需要角色继承 / 部门范围权限(`parentRoleId`) | `Role.parentId` 字段 + 权限合并算法 | B-15(新),走 ADR |
| 需要 access token 主动吊销 / 异常 refresh 重放全链路吊销 | Redis 黑名单 / RefreshToken family / reuse detection | B-7 残余,走 ADR(联动 B-10) |

**判定原则**:不是"觉得以后会用",而是"现在的产品需求里出现了这个明确诉求"。

**v2.0 baseline 已覆盖的能力(下列条目在 v1 历史阶段属于 §9 升级路径,v2.0 起属 baseline)**:

- ~~真要做"权限点到按钮级" → 加 `permissions` 表 + `casl` 库~~ → **v2.0 baseline 已升级**(不引入 CASL,自写;见 ADR-001 §4)
- ~~真有"无感续期"诉求 → 加 refresh token 表 + 接口~~ → **v2.0 baseline 已升级**(短 TTL access + 长 TTL refresh + rotation + logout;见 ADR-001 §4.7)
...
> **v2.0 baseline 升级提示**:本章节描述 V1.1 工程加固阶段(已收尾,历史任务清单见 [`docs/release-tasks/v1.1-engineering-hardening.md`](./docs/release-tasks/v1.1-engineering-hardening.md))。其中"V1.1 不引入 RBAC、不引入 refresh token"是 **V1.1 阶段的历史约束**,**不适用 v2.0 baseline**;v2.0 已把 RBAC + refresh token + logout 升级为底座必备能力(见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md))。本章节正文保留供 V1.1 历史脉络参考,不再作为 v2.0 baseline 的约束。
...
>
> **与 §9 升级路径的关系**:§9 列出的所有"升级触发条件"在 V1.1 阶段**仍然不触发**;V1.1 不引入 Redis、不引入 BullMQ、不引入 RBAC、不引入 refresh token、不引入文件上传 Provider、不引入 LLM、不引入审计日志表、不引入用户状态缓存。**(本声明是 V1.1 历史阶段约束;v2.0 baseline 已升级 RBAC + refresh token,见本节顶部 v2.0 提示)**
...
## 12. v2.0 baseline 升级(RBAC + Auth)

> **本章节是 ARCHITECTURE.md 第一个 v2.0 章节**,与 §1-§11 共存。v2.0 的设计权威依据是 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md);本章节是从 ADR-001 提炼的最小语义索引,**详细规则以 ADR-001 为准**。

### 12.1 v2.0 一句话总结

v2.0 是模板自身的 **baseline 重置版本**(breaking change),把 v1 历史阶段列为"不做"的 **RBAC + refresh token + logout** 升级为模板必备能力,采用 `User → Role → Permission` 四表模型 + 短 TTL access + 长 TTL refresh + rotation + logout。**不引入 CASL,不做多租户 / 组织树 / 数据权限 / 部门继承 / 部分通配权限 / `@AnyPermission` / access token 主动吊销 / 异常 refresh 全链路吊销**(这些走 [`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §B 的 B-1/2/12/13/14/15 + B-7 残余 ADR 解锁)。

### 12.2 红线分级(替代 v1 阶段"v1 不做"清单)

| 档位 | 含义 | 来源 |
|---|---|---|
| **永久红线** | 任何 ADR / 派生项目 / v2.x 后续版本都不能削弱 | [`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §A(v2.0 扩展至 29 条) |
| **v2.0 baseline 能力** | 模板必须提供,不允许跳过 | ADR-001 §1.2 + 本节 §12.3 |
| **派生项目 ADR 解锁能力(B 类)** | 派生项目按需走 ADR | [`docs/capability-unlock-matrix.md`](./docs/capability-unlock-matrix.md) §B |

### 12.3 v2.0 baseline 必备能力

- **RBAC 四表**:`Role` / `Permission` / `UserRole` / `RolePermission`(详见 ADR-001 §4.1)
- **三个种子角色**:`super_admin`(`*`)/ `admin`(`user:read` + `user:write` + `role:read` + `permission:read`)/ `user`(空)
- **七条种子权限**:`*` / `user:read` / `user:write` / `user_role:assign` / `role:read` / `role:write` / `permission:read`
- **权限码命名严格** `<resource>:<action>` 两段式,不允许"唯一例外"(永久红线 §A 第 27 条)
- **`@Permissions(...)` 装饰器**(AND 语义)+ **`PermissionsGuard`** 全局注册(替代 v1 `@Roles` + `RolesGuard`)
- **`@AnyPermission(...)` OR 语义** baseline 不做,走 ADR B-13 解锁(**不是永久禁止**)
- **JwtPayload 字段** `{ sub, username, typ: 'access' | 'refresh', jti }`,不塞 `roles` / `permissions`
- **Refresh token 表 `RefreshToken`**,`tokenHash` sha256 存储(永久红线 §A 第 11 条)
- **Refresh rotation 强制**:每次 refresh 旧 token 立即 `revokedAt`
- **Logout 语义固定**:`POST /api/auth/logout` 要求 access token 登录 + 提交 refreshToken,只吊销当前用户对应的该条(详见 ADR-001 §9.3)
- **五条 super_admin 铁律**(详见 ADR-001 §4.5):分配限制 / 最后一个保护 / 自我保护
- **创建用户防权限提升**:`POST /api/users` 缺省 `roleCodes: ['user']`;显式非缺省值需 `user_role:assign`(详见 ADR-001 §9.4)
- **`@Public()` 与 `@Permissions(...)` 互斥** + 启动期检测 + E2E 守护(详见 ADR-001 §9.5)

### 12.4 v2.0 实施阶段(本文档当前位于 Phase 0)

| Phase | 内容 | 文件触达范围 |
|---|---|---|
| **Phase 0(当前)** | 文档与 ADR-001 | `ARCHITECTURE.md` / `CLAUDE.md` / `AGENTS.md` / `docs/capability-unlock-matrix.md` / `CHANGELOG.md` + 本 ADR;**不动** `src` / `prisma` / `package.json` / migration |
| Phase 1 | Prisma schema + migration + seed | `prisma/schema.prisma` + 新 migration + `prisma/seed.ts` |
| Phase 2.1-2.6 | 代码层(装饰器 / Guard / Strategy / users / roles / permissions / auth 扩展 / BizCode) | `src/**` |
| Phase 3.1-3.2 | E2E 全量重写 + OpenAPI 契约快照重生 | `test/**` |
| Phase 4 | CHANGELOG v2.0.0 release note + capability-matrix 状态全表刷新 | release |

**Phase 之间独立 commit**;合并主线前先打 `v1.x-maintenance` 分支供派生项目按 v1 兜底。

> **Phase 1+2+3 合并实施策略(基于 Phase 0 后做的可行性核查修订)**:Phase 1 单独 PR 不可合并 `main`(删 `User.role` + 删 `Role` enum 会让 18 个文件 + 100+ 引用点同步 typecheck 红)。实际可合并的 PR 粒度修订为 **Phase 1+2+3 同一 PR 分 7 个逻辑 commit**(Phase 4 仍单独 PR),完整决策与 commit 边界、合并方式、branch protection 处理见 [`docs/adr/ADR-001-v2-rbac-auth-baseline.md`](./docs/adr/ADR-001-v2-rbac-auth-baseline.md) §7.1A。**Required checks 始终不动**,只在 PR 最终 HEAD CI 全绿后合并;中间 commit 不允许单独合并 / cherry-pick 到 `main`。

### 12.5 派生项目 v1 → v2 升级建议

派生项目按自身数据量与停机窗口写一次性数据迁移(详见 ADR-001 §7.2):

1. 部署 v2 代码 + 临时保留旧 `User.role` 字段(过渡 schema)
2. 跑数据迁移:每个 user 按旧 `role` 字段创建对应 `UserRole`(`SUPER_ADMIN`→`super_admin` 等)
3. 验证业务读写全部走新模型
